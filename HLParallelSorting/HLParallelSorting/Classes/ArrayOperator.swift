...
public class ArrayOperator {
    ...
    public var array = [Float]() {
        didSet {
            if array.count>0 {
                arrayBuffer = self.function.device?.makeBuffer(bytes: self.array, length: MemoryLayout.size(ofValue: self.array), options: .storageModeShared /*MTLResourceOptions()*/)
                var size = array.count
                arraySizeBuffer = self.function.device?.makeBuffer(bytes: &size, length: MemoryLayout.size(ofValue: size), options: .cpuCacheModeWriteCombined)
            }
        }
    }
    ...
    public func run(){
        if let buffer = arrayBuffer {
            ...
            function.execute(size: array.count) { (commandEncoder) in
