...
public func test() {
   ...
   // print("# Sorting of {...n} ∈ ℝ:\tGPU.time = \((t31-t30)/TimeInterval(times)), CPU(qsort).time = \((t51-t50)/TimeInterval(times)), CPU(swiift3).time = \((t61-t60)/TimeInterval(times)),  but: DSP.time = \((t41-t40)/TimeInterval(times))!!! ")
}
